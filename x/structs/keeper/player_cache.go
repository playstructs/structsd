...
func (cache *PlayerCache) GetGuildRank() uint64 {
	if !cache.PlayerLoaded {
		cache.LoadPlayer()
	}
	return cache.Player.GuildRank
}
...
func (cache *PlayerCache) GetGuildId() string {
	if !cache.PlayerLoaded {
		cache.LoadPlayer()
	}
	return cache.Player.GuildId
}
