...
func (k msgServer) PlayerCreateProxy(goCtx context.Context, msg *types.MsgPlayerCreateProxy) (*types.MsgPlayerCreateProxyResponse, error) {
    ...
    if (!guildFound) {
        return &types.MsgPlayerCreateProxyResponse{}, sdkerrors.Wrapf(types.ErrObjectNotFound, "Referenced Guild (%d) not found", guild.Id)
    }
    ...
    if (!k.PermissionHasOneOf(ctx, addressPermissionId, types.PermissionAssociations)) {
        return &types.MsgPlayerCreateProxyResponse{}, sdkerrors.Wrapf(types.ErrPermissionManageGuild, "Calling address (%s) has no Guild Management permissions ", msg.Creator)
    }
	...
	if (msg.SubstationId != "") {
        ...
        if (!substationFound) {
            return &types.MsgPlayerCreateProxyResponse{}, sdkerrors.Wrapf(types.ErrObjectNotFound, "Provided Substation Override (%s) not found", msg.SubstationId)
        }
        ...
        if (!k.PermissionHasOneOf(ctx, substationObjectPermissionId, types.PermissionGrid)) {
            return &types.MsgPlayerCreateProxyResponse{}, sdkerrors.Wrapf(types.ErrPermissionGuildRegister, "Calling player (%d) has no Player Connect permissions on Substation (%s) used as override", proxyPlayer.Id, substation.Id)
        }
	}
    ...
    if (!substationFound) {
        ...
        if (!substationFound) {
            return &types.MsgPlayerCreateProxyResponse{}, sdkerrors.Wrapf(types.ErrObjectNotFound, "Entry Substation (%s) for Guild (%s) not found", guild.EntrySubstationId, guild.Id)
        }
    }
