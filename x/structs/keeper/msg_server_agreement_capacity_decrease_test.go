...
func TestMsgAgreementCapacityDecrease(t *testing.T) {
	...
	testCases := []struct {
		name      string
		input     *types.MsgAgreementCapacityDecrease
		expErr    bool
		expErrMsg string
		skip      bool
	}{
		{
			name: "valid capacity decrease",
			input: &types.MsgAgreementCapacityDecrease{
				Creator:          player.Creator,
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			resp, err := ms.AgreementCapacityDecrease(wctx, tc.input)
