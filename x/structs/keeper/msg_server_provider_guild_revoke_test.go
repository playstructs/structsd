...
func TestMsgProviderGuildRevoke(t *testing.T) {
	...
	testCases := []struct {
		name      string
		input     *types.MsgProviderGuildRevoke
		expErr    bool
		expErrMsg string
		skip      bool
	}{
		{
			name: "valid guild revoke",
			input: &types.MsgProviderGuildRevoke{
				Creator:    providerOwner.Creator,
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.name == "valid guild revoke" {
				...
				// Grant guild first
				_, _ = ms.ProviderGuildGrant(wctx, &types.MsgProviderGuildGrant{
					Creator:    providerOwner.Creator,
					ProviderId: provider.Id,
					GuildId:    []string{guild.Id},
				})
			...
			resp, err := ms.ProviderGuildRevoke(wctx, tc.input)
