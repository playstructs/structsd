...
func TestMsgGuildMembershipInviteApprove(t *testing.T) {
	...
	// Grant permissions
	addressPermissionId := keeperlib.GetAddressPermissionIDBytes(targetPlayer.Creator)
	...
	testCases := []struct {
	...
	}{
		{
			...
			input: &types.MsgGuildMembershipInviteApprove{
				Creator:  targetPlayer.Creator, // The player being invited approves their own invite
				GuildId:  guild.Id,
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			// Create invite first
			inviterAddressPermissionId := keeperlib.GetAddressPermissionIDBytes(inviter.Creator)
			testPermissionAdd(k, ctx, inviterAddressPermissionId, types.PermGuildMembership)
			...
			resp, err := ms.GuildMembershipInviteApprove(wctx, tc.input)
