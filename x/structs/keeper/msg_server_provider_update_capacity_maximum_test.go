...
func TestMsgProviderUpdateCapacityMaximum(t *testing.T) {
	...
	testCases := []struct {
		name      string
		input     *types.MsgProviderUpdateCapacityMaximum
		expErr    bool
		expErrMsg string
	}{
		{
			name: "valid capacity maximum update",
			input: &types.MsgProviderUpdateCapacityMaximum{
				Creator:            player.Creator,
				ProviderId:         provider.Id,
				NewMaximumCapacity: 2000,
			},
			expErr: false,
		},
		{
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.name == "valid capacity maximum update" {
				provider, _ = k.AppendProvider(ctx, provider)
				tc.input.ProviderId = provider.Id
			}

			resp, err := ms.ProviderUpdateCapacityMaximum(wctx, tc.input)
			...
			if tc.expErr {
				...
				// Verify capacity maximum was updated
				updatedProvider, found := k.GetProvider(ctx, provider.Id)
				require.True(t, found)
				require.Equal(t, tc.input.NewMaximumCapacity, updatedProvider.CapacityMaximum)
			}
		})
	}
}
