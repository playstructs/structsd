...
//     MsgGuildBankMint path. An optional minAmountToken guards against ratio
...
//     the two per-guild fee rates (LegacyDec, range 0.0-1.0). The fee is always
...
//
//   - MsgGuildBankRedeem now applies the guild's convert-out fee and accepts an
//     optional minAmountAlpha slippage guard. Its pro-rata payout is also
...
//     independently hardened (GuildCache.LoadGuild and InitGenesis normalize nil
//     fees to zero), so the migration is defense-in-depth for live state.
